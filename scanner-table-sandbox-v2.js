...
(function () {
  ...
  function applyFilters(list) {
    const minProb = num(byId('minProb') && byId('minProb').value, 0), rocMin = num(byId('rocMin') && byId('rocMin').value, -Infinity), rocMax = num(byId('rocMax') && byId('rocMax').value, Infinity), ivMin = num(byId('ivMin') && byId('ivMin').value, -Infinity), oiMin = num(byId('minOI') && byId('minOI').value, -Infinity), shortOiMin = num(byId('minShortOI') && byId('minShortOI').value, -Infinity), maxSpread = num(byId('maxSpread') && byId('maxSpread').value, Infinity), width = num(byId('spreadWidth') && byId('spreadWidth').value, 0);
    const dteParts = String(byId('dteWindow') ? byId('dteWindow').value : '0-45').match(/(\d+)\D+(\d+)/), dteMin = dteParts ? Number(dteParts[1]) : 0, dteMax = dteParts ? Number(dteParts[2]) : 45;
    return list.filter(r => { if (!includeMoreRecords && r.strikeValidationStatus !== 'PASS') return false; if (lowerAsPercent(r) < minProb) return false; if (num(r.rocAfterCommissionAndFees, -Infinity) < rocMin || num(r.rocAfterCommissionAndFees, Infinity) > rocMax) return false; const iv = Math.abs(num(r.monthlyChainIV, 0)) <= 1 ? num(r.monthlyChainIV, 0) * 100 : num(r.monthlyChainIV, 0); if (iv < ivMin) return false; if (num(r.openInterest, 0) < oiMin || num(r.shortPutOI, 0) < shortOiMin || num(r.shortCallOI, 0) < shortOiMin) return false; if (num(r.spreadMax, Infinity) > maxSpread) return false; if (width > 0 && num(r.requestedWidth) !== width) return false; if (num(r.dte, -Infinity) < dteMin || num(r.dte, Infinity) > dteMax) return false; return true; });
  }
