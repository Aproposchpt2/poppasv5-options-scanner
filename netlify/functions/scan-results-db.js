...
function calcBias(symbol, candles) {
  ...
  const blank = { directionalBias:'Neutral', biasScore:0, biasMethod:'Hybrid Price-Action Score', biasReason:'Insufficient data', biasAsOf:today };
  if (!candles || candles.length < 35) return blank;
  ...
  if(pH.length>=2&&pL.length>=2){
    const bull=pH[pH.length-1]>pH[pH.length-2]&&pL[pL.length-1]>pL[pL.length-2];
    const bear=pH[pH.length-1]<pH[pH.length-2]&&pL[pL.length-1]<pL[pL.length-2];
    if(bull){c2=1;c2txt='pivot structure is bullish';}else if(bear){c2=-1;c2txt='pivot structure is bearish';}else{c2txt='pivot structure is mixed';}
  }
  const r20H=Math.max(...highs.slice(-20)),r20L=Math.min(...lows.slice(-20)),rng=r20H-r20L;
  let c3=0,c3txt='price is mid-range';
  if(rng>0){const pos=(close-r20L)/rng;if(pos>=0.60){c3=1;c3txt='price is in upper range';}else if(pos<=0.40){c3=-1;c3txt='price is in lower range';}}
  const score=c1+c2+c3, bias=score>=2?'Bullish':score<=-2?'Bearish':'Neutral';
  const reason=score===3?'Trend, pivot structure, and range location all confirm upside posture.':score===-3?'Price below trend averages, lower highs and lower lows, and in lower range.':`${(c1txt.charAt(0).toUpperCase()+c1txt.slice(1))}, ${c2txt}, and ${c3txt}.`;
...
export default async (req) => {
  try {
    ...
    // Enrich rows with Directional Bias (Yahoo Finance candles, per unique symbol)
    const symbols = [...new Set(rows.map(r => r.symbol).filter(Boolean))];
    const candleMap = new Map();
    await Promise.allSettled(symbols.map(async sym => {
      candleMap.set(sym, await fetchCandles(sym));
    }));
    rows = rows.map(r => ({ ...r, ...calcBias(r.symbol, candleMap.get(r.symbol)) }));
